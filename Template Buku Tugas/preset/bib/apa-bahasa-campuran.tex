...
\renewcommand{\BOthersPeriod}[1]{\textit{et~al}.\hbox{}}% ``et al'' dengan titik
...
\renewcommand{\PrintOrdinal}[1]{%
    \edef\APACtemp{#1}%
    \if\relax\detokenize\expandafter{\APACtemp}\relax
    ??%
    \else
    \afterassignment\APACtest@num
    \count@=0\APACtemp\relax
    \fi
}
\def\APACtest@num#1\relax{%
    \ifx\relax#1\relax
    % number
    edisi~ke-\the\count@
    \else
